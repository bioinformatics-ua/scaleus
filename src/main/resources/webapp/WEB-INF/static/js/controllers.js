...
app.controller('NamespacesCtrl', function ($scope, NamespacesService, SharedService) {
	...
	$scope.getNamespaces = function () {
		if (SharedService.selectedDataset) {
			NamespacesService.get({dataset: SharedService.selectedDataset}, function (response) {
				...
				angular.forEach($scope.namespaces, function (val, key) {
					$scope.modelContainer.push({item: {prefix: key, namespace: val}, checked: false});
				});
			}, function (response) {
				// an error occured
	...
	$scope.addNamespace = function () {
		console.log('Put namespace');
		if ($scope.formPrefix && $scope.formNamespace) {
			var ns = {'prefix': $scope.formPrefix, 
					  'namespace': $scope.formNamespace}
			NamespacesService.add({dataset: SharedService.selectedDataset}, ns, function (response) {
				console.log(response);
				$scope.getNamespaces();
				$scope.formPrefix = "";
				$scope.formNamespace = "";
...
app.controller('TriplesCtrl', function ($scope, TriplesService, SharedService) {
	...
	$scope.addTriple = function () {
		...
		if ($scope.formSubject && $scope.formPredicate && $scope.formObject) {
			...
			var triple = {'s': $scope.formSubject, 
						  ...
						  'o': $scope.formObject};
			console.log(triple);
			TriplesService.add({dataset: SharedService.selectedDataset}, triple, function (response) {
				console.log(response);
				$scope.formSubject = "";
				$scope.formPredicate = "";
				$scope.formObject = "";
	...
	$scope.removeTriple = function () {
		...
		if ($scope.formSubject && $scope.formPredicate && $scope.formObject) {
			...
			TriplesService.delete({dataset: SharedService.selectedDataset, triple: triple}, function (response) {
